...
def mergeranges(ranges):
    ...
    import numpy as np
    if len(ranges) < 2: #leaves numpy array as if it is smaller than two lines
        return ranges
    for i in ranges:
        if i[0] > i[1]:
            i[1], i[0] = i[0], i[1]
...
def readbasecfg(f, v):
    ...
    if f == '':
        return cfg #returns the above specified config file if empty string is passed for f to the function
    with open(f, 'r') as fin:
        ...
        for line in fin:
            ...
            if line[0] not in cfgk:
                ...
                continue
            if line[0] in ['syncol', 'invcol', 'tracol', 'dupcol', 'delcol']: #SEEK
                try:
...
def readfasta(f): #reads genomic sequences
    from gzip import open as gzopen
    from gzip import BadGzipFile
    from collections import deque #an alternative to lists
    import sys
    NUCBP = 'ACGTRYSWKMBDHVN' #defines nucleotides
    NUCBP += NUCBP.lower() #puts them in lower case as well
    out = {}
    chrid = ''
    chrseq = deque()
...
def readannobed(path, v, chrlengths): #Readung bed file, irrelevant for us because we are not making tracks
    from collections import deque
    import logging
...
def readsyriout(f):
    ...
    syri_regs = deque()
    skipvartype = ['CPG', 'CPL', 'DUPAL', 'HDR', 'INS', 'INVAL', 'INVDPAL', 'INVTRAL', 'NOTAL', 'SNP', 'SYNAL', 'TDM', 'TRANSAL'] #SEEK
    logger.info('Reading input files generated by syri.')
    with open(f, 'r') as fin:
        for line in fin:
            l = line.strip().split()
            # TODO: DECIDE WHETHER TO HAVE STATIC VARS OR FLEXIBLE ANNOTATION
            if l[10] in VARS: #adds the current line to syri_regs
                syri_regs.append(l)
            else:
    ...
    if df.shape[1] == 13: #checking if there are 13 columns
        ...
        df[12] = '-'
    df = df[[0, 1, 2, 5, 6, 7, 10, 12]] #only selecting some columns from the syri.out file
    colnames = ['achr', 'astart', 'aend', 'bchr', 'bstart', 'bend',  'type', 'anno'] #naming the columns
    df[[0, 5, 10]] = df[[0, 5, 10]].astype(str) #making the achr, bchr and type columns str
    try:
        df[[1, 2, 6, 7]] = df[[1, 2, 6, 7]].astype(int) #making astart aend bstart bend integer
    except ValueError:
...
def filterinput(args, df, chrid, itx=False): #filters the dataframe of syri.out based on if the nosyn etc options are passed
